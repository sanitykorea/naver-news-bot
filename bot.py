...
# 기업/관공서 홍보성 기사에 흔한 제목·요약 어휘 — 어느 키워드든 걸리면 제외.
# "공모" 단독은 안 넣는다 — "뇌물 공모"처럼 정치 기사에도 흔히 쓰이는 말이라
# 구체적인 구절("명칭 공모")로만 넣는다.
BUSINESS_NOISE = ("증권", "수출", "수주", "호재", "사회공헌", "관련주", "주가", "강세", "경영",
                  "특징주", "스폰서십", "사모펀드", "지분", "한전MCS",
...
EXCLUDE = {"녹색당": ("영국", "英", "런던", "잉글랜드", "스코틀랜드", "웨일스",
                     ...
                     "캐나다", "加", "토론토", "밴쿠버", "오타와",
                     "독일", "獨", "베를린", "뮌헨", "함부르크"),
           # 메타·틱톡 등 해외 빅테크 상대 집단소송 뉴스가 "SNS"에 걸려온다.
           # "소송" 단독은 전역으로 안 막는다 — 인권·노동 기사에 흔한 말이라, 이 두
           # 키워드에만 국한해서 막는다.
           "청소년 SNS": ("소송",),
           "청소년 스마트폰": ("소송",)}
...
RELEVANCE_CHECK = {
    ...
    "탄소중립법": (("탄소중립법", "탄소중립기본법"), ()),
    # "환경"은 "전략 환경이 어렵다"처럼 생태와 무관한 "상황/여건" 뜻으로도 흔히 쓰이는
    # 동음이의어다. 행위자 개념이 없는 단어라 aliases는 비워두고, 진짜 환경 어휘가
    # 하나라도 같이 나올 때만 통과시킨다.
    "환경": ((), ("기후", "환경오염", "환경부", "탄소", "온실가스", "미세먼지", "생태",
                "오존", "폐기물", "재생에너지", "대기질", "수질", "환경단체", "환경운동",
                "생물다양성")),
}
...
def selftest():
    ...
    # "환경": "상황/여건" 뜻이면 제외, 진짜 생태 어휘가 있으면 통과
    assert excluded("환경", "조현 장관 한일 관계 정부에만 맡길 수 없어",
                     ["안보 외교 에너지 과학기술 분야에서 관계가 중요하다" * 2]) == "접점없음"
    assert excluded("환경", "환경부 미세먼지 대책 발표",
                     ["환경부는 오늘 미세먼지 저감 대책을 발표했다" * 2]) is None
    assert excluded("청소년 SNS", "메타 '청소년 SNS 중독' 소송 합의…23조 원 지급", []) == "제목"
    assert excluded("청소년 SNS", "청소년 SNS 이용 실태 조사 결과 발표", []) is None
